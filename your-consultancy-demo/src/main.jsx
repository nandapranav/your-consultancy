...
function App(){
 ...
 return <div className="app">
  ...
  <AnimatePresence>{selected&&<motion.div className="modalBack" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setSelected(null)}><motion.div className="modal" initial={{y:30,opacity:0}} animate={{y:0,opacity:1}} exit={{y:30,opacity:0}} onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}><X/></button><div className="sectionKicker">{selected.label}</div><h2>{selected.title}</h2><p>{selected.result}</p><div className="modalMetric"><b>{selected.metric}</b><span>{selected.metricLabel}</span></div><div className="tags">{selected.tags.map(t=><span key={t}>{t}</span>)}</div></motion.div></motion.div></AnimatePresence>}
 </div>
}
